...
class TencentGetData():
    ...
    def __init__(self, specify_date=None):
        ...
        self.soup = BeautifulSoup(open("Tencent_data/%s.htm"%date),'lxml')
    ...
    def getAssetDetails(self):
        ...
        total_assest_node = assest_box_node.find('p',"num js-data-product_balance")
        self.total_asset = float(total_assest_node.get_text())
    ...
    def createAssetTable(self):
        ...
        asset_table = pd.DataFrame(index=['总资产','昨日固定收益','累计固定收益','昨日浮动收益','累计浮动收益','实时年化收益%'])
        roa = (self.fix_income_yes_pnl + self.float_income_yes_pnl)/self.total_asset*300*100
        asset_table[self.date] = [self.total_asset, self.fix_income_yes_pnl, self.fix_income_sum_pnl, 
                   self.float_income_yes_pnl, self.float_income_sum_pnl, round(roa,2)]
